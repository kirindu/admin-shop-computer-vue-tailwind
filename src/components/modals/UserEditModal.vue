...
const handleSubmit = async () => {
  ...
  if (validateForm()) {
    loading.value = true;
    try {
      const { data } = await ShopAPI.editUser(props.user._id, jsonPayload);
...
<template>
  <div class="vh-100" style="background-color: #1e293b">
    <SectionMain style="color: snow">
      <SectionTitleLineWithButton :icon="mdiBallotOutline" title="Edit User" main>
