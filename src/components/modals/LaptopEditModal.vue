...
const form = reactive({
  name: props.laptop.name,
  RFID: props.laptop.RFID,
  notes: props.laptop.notes,
  BeaconUID:"none"
});
...
const handleSubmit = async () => {
  ...
  if (validateForm()) {
    loading.value = true;
    try {
      const { data } = await ShopAPI.editShopComputer(props.laptop._id, jsonPayload);

      if (data.ok) {
        showSweetAlert({
          title: 'Shop Computer updated successfully!',
          icon: 'success',
        ...
        await showSweetAlert({
          title: 'Shop Computer no updated!',
          text: data.msg,
          icon: 'warning',
          showCloseButton: true,
          allowOutsideClick: false
